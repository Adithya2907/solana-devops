...
<div class="content">
			...
			{#if data.repo}
				...
				<ul>
					{#each data.repo.listeners as listener}
						<li><i class="fa-solid fa-ear-listen" />{listener.branch} on {listener.type}</li>
					{/each}
				...
				<ul>
					{#each builds as build}
						<li>
							...
							<a href="/repo/{data.repo?.name}/build/{build.id}"
								>Build {build.id}
	...
	<form method="POST" action="?/listener:add" use:enhance>
		...
		<input bind:checked={deployfe} name="deployfe" type="checkbox" />
