<script lang="ts">
    ...
    export let data;
	import { IdlTypeToString, IsMut, IsSigner, type CustomAccountType, type Instruction } from '../IDL';
    import type {IDL} from '../IDL';
    let idl: IDL;

    if (data.idlJson == 'ERROR') error = true;
	else idl = JSON.parse(data.idlJson);

    // populating the accounts and arguments arrays inside each instruction
    idl.instructions.forEach((instruction: Instruction) => {
        ...
        if(instruction.args.length > 0) instruction.argumentvalues = new Array<string>(instruction.args.length)
    });
   ...
   const localStorageKey = 'walletAdapter';
        ...
        if(idl.instructions[instructionIndex].accountValues) {
            idl.instructions[instructionIndex].accountValues?.forEach((customAccount, i) => {
                if(idl.instructions[instructionIndex].accountValues[i].generateKeypair) {
                    ...
                    accounts[idl.instructions[instructionIndex].accounts[i].name] = idl.instructions[instructionIndex].accountValues[i].value;
    ...
    const accountTypes = [{name: "Public Key", value: 0}, {name: "Generate Keypair", value: 1}, {name: "System Program", value: 2}]
                    ...
                    <form class="instruction">
                        ...
                        {#each instruction.accounts as account, ac}
                            <div class="account" style="margin-bottom: 5px">
                                ...
                                <input placeholder="Enter public key" bind:value={idl.instructions[i].accountValues[ac].value} type="text">
                                <select bind:value={idl.instructions[i].accountValues[ac].accountType} on:change={() => selectHandler(i, ac)}>
